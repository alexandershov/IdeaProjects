...
def binary_search_right(seq, value):
    """
    Return max index where value would be located after insertion
    """
    start = 0
    end = len(seq)
    # invariants are:
    ...
    # so everything after and including end is > value
    # everything before start (== end) is <= value
    # so end (== start) is the first index where seq[end] > value
    # if value is in seq, then it's 1 + index of last occurrence
    # if value is not in seq then it's just insertion point
    return start
...
def test_binary_search_right(items, value, expected):
    assert binary_search_right(items, value) == expected
